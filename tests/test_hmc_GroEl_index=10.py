...
posterior, gibbs = create_replica(dataset, K, beta, index, n_steps=n_steps)
...
path = '/home/nvakili/Documents/PYTHON_CODES/weekly_experiments/24.5.2017/marginalR/'
marginalR = np.load(os.path.join(path,'marginalR_100.npz'))['arr_0']
norms  =[clouds.frobenius(marginalR[index], x[-1][-1]) for x in samples]
log_posterior3 = np.array(rex.prob_posterior)
log_prior3 = np.array(rex.prob_prior)
    
log_L3 = log_posterior3 - log_prior3
