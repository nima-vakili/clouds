...
n_iter     = 100
K          = 5#0
dataset =  'toy_tau=1_K=5'#('1aon_hmc_test')
...
if False:
    #        norms  =[clouds.frobenius(marginalR[index], x[-1][-1]) for x in samples]
        ...
        s1 = hmc.MarginalPosteriorMeans_hmc(params,L)
        steps_mu = s1.sample()
        s2 = hmc.MarginalPosteriorRotations_hmc(params,L)
        steps_R = s2.sample() 
...
if False:
    ...
    INDEX = 24
    path = '/home/nvakili/Documents/PYTHON_CODES/weekly_experiments/23.15.2017/results_hmc_GroEl_index/index_'+str(INDEX)+'/'
