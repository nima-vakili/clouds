...
if __name__ == '__main__':
    ...
    for index in range(params.n_projections):
        ...
        for i in range(len(beta)):
            ...
            posteriors[index].append(posterior)
            replicas[index].append(gibbs)
        ...
        if False:
            rex = ReplicaExchangeMC(posteriors[index], replicas[index])
        else:
            rex = RotationReplicaExchange(posteriors[index], replicas[index])
        n_samples = 1000
        
        states[index].append([get_state(replica.pdfs[0].params) for replica in replicas[index]])
        initial[index].append(map(deepcopy, states[index]))
...
#        pr = cProfile.Profile()
#        pr.runcall(threaded(rex.run, states, n_samples, verbose= True))
#        pr.dump_stats('test_hmc_GroEl_profiling')
#        stats = pstats.Stats('test_hmc_GroEl_profiling')
#        stats.strip_dirs()
