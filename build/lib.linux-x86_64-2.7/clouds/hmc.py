...
from .marginal import MarginalLikelihood_R, RotationMaximizer, MarginalLikelihood_mu
...
class EulerEnergy(MarginalLikelihood_R):
...
#class MeanEnergy(MarginalLikelihood_mu):
#    
#    def gradient(self, mu):
#        mu_old = self.params.mu.copy()
#        self.params.mu[...] = mu
#        grad = super(MeanEnergy, self).gradient()            
...
#        for i in range(self.n_steps):   
        ...
        self.acc_rates.append(hmc.acc_rates)
...
class MarginalPosteriorRotations_hmc(Posterior, PriorRotations):
    ...
    def __init__(self, params, likelihood, verbose=False):
        ...
        self.optimize = False
        self.stepsize_ind = np.ones(self.params.n_projections) * .1
...
#        self._likelihoods = MeanEnergy(params=params)
...
class MarginalPosteriorMeans_hmc_Version2(Posterior, PriorMeansVersion2):
    
    def __init__(self, params, likelihood):

        super(MarginalPosteriorMeans_hmc_Version2, self).__init__(params, likelihood)

        pdfs = [MarginalLikelihood_mu(params=params), PriorMeansVersion2(params=params)]
        self._likelihoods = MeanEnergy(params, pdfs)
#        self._likelihoods = MeanEnergy(params=params)
        self._sampler = MeanSampler(n_steps=10, stepsize=1e-1)
